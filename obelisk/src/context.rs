...
impl<'a> Context<'a> {
    pub fn render(&self, template_root: &str, deploy_prefix: &str) {
        ...
        match self {
            Posts(posts) => {
                let post_template_path = &format!("{}/templates-handlebars/post.html", template_root);
                let post_template = PostTemplate::new(post_template_path);
            ...
            Tags(tags) => {
                let tags_template_path = format!("{}/templates-handlebars/tags.html", template_root);
                let tags_template = TagsTemplate::new(&tags_template_path);
            ...
            Index(index) => {
                let index_template_path = format!("{}/templates-handlebars/index.html", template_root);
                let index_template = IndexTemplate::new(&index_template_path);
                let rendered = index_template.render(&index);
                std::fs::write(&format!("{}/index.html", deploy_prefix), rendered)
                    .expect("failed to write post to deploy");
            }
            Rss(rss) => {
                let rss_template_path = format!("{}/templates-handlebars/rss.xml", template_root);
                let rss_template = RssTemplate::new(&rss_template_path);
                let rendered = rss_template.render(&rss);
                std::fs::write(&format!("{}/rss.xml", deploy_prefix), rendered)
                    .expect("failed to write post to deploy");
            }
            Sitemap(sitemap) => {
                let sitemap_template_path = format!("{}/templates-handlebars/sitemap.xml", template_root);
                let sitemap_template = SitemapTemplate::new(&sitemap_template_path);
